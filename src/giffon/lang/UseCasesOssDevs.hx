...
class UseCasesOssDevs {
    static public function titleTag(lang:Language) return switch (lang) {
        ...
        case Chinese: 'Giffon怎樣幫助開源軟件開發者?';
    }
    ...
    static public function introText(lang:Language) return switch (lang) {
        ...
        case Chinese: jsx('
            <Fragment>
                <p>
                    好多開發者都會用自己的空閒時間做開源項目. 
                    而這些開源項目對好多大小公司都很有用. 但大多數開源項目本質上都是免費, 開發者只做開源項目實在難以維持生計. 
                    往往開發者在缺乏動力的情況下, 這些開源項目就慢慢停濟不前無人維護 <span className="ec ec-weary"></span>.
                </p>
                <p>
                    Giffon給用家一個途徑去向開發者表達謝意. 
    ...
    static public function couponDetail(lang:Language) return switch (lang) {
        ...
        case Chinese: jsx('
            <Fragment>
                <p>在你的願望輸入 <span className="d-inline border bg-light p-1">OSS_ROCKS_JULY</span> 就可以立即從Giffon得到35港元或5美金的支持.</p>
                <p className="text-muted mb-0">只適用於有連結GitHub或GitLab帳號的用家. 2019-07-31前使用. 限額: 50人.</p>
            </Fragment>
        ');
    }
    ...
    static public function ideLicenseDescription(lang:Language) return switch (lang) {
        ...
        case Cantonese | Chinese: '用專業版開發軟件, 事半功倍. JetBrains? Sublime? Visual Studio? 隨你喜歡.';
    }
    ...
    static public function devSwagsDescription(lang:Language) return switch (lang) {
        ...
        case Chinese: '在你的手提電腦貼上你喜愛的開發項目logo貼紙, 着上IT品牌T-shirt, 抱一下IT吉祥物公仔.';
    }
    ...
    static public function serviceFeeAmountDescription(lang:Language) return switch (lang) {
        ...
        case Chinese: '服務費是願望總值的 ${(giffon.ChargeInfo.serviceChargeRate*100).trim().toString()}%, 由支持者湊合支付. Giffon不會收取開發者任何費用.';
    }

    static public function projectTeamBased(lang:Language) return switch (lang) {
        case English: 'Project/team-based';
        case Cantonese | Chinese: '適合個別項目/開發團隊';
    }
    ...
    static public function projectsTeamsReceiveMoney(lang:Language) return switch (lang) {
        ...
        case Cantonese | Chinese: '項目/團隊收錢';
    }
    ...
    static public function projectsTeamsReceiveMoneyDescription(lang:Language) return switch (lang) {
        ...
        case Chinese: '項目/團隊可以表示會怎樣使用資金, 但無任何機制驗證.';
    }
    ...
    static public function openCollectiveFees(lang:Language) return switch (lang) {
        ...
        case Cantonese | Chinese: '5或10%服務費 + 支付處理費 + 提款費用';
    }
    ...
    static public function gitHubSponsorsFeesDescription(lang:Language) return switch (lang) {
        ...
        ');
        case Chinese: jsx('
            <Fragment>GitHub Sponsors面世後第一年免費, 但GitHub已<a href="https://help.github.com/en/articles/about-github-sponsors#about-the-github-sponsors-matching-fund" target="_blank" rel="noopener">表明</a>期後會收取費用.</Fragment>
        ');
    }
}